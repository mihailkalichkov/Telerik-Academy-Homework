...
class AssertionsHomework
{
    ...
    {
        Debug.Assert(arr != null, "The array is empty");
        ...
        where T : IComparable<T>
    {
        ...
        // assert that the found element is indeed the smallest
        for (int i = startIndex + 1; i < endIndex; i++)
        {
    ...
    public static int BinarySearch<T>(T[] arr, T value) where T : IComparable<T>
    {
        ...
        //assert that the array is sorted - Binary Search works only on sorted arrays
        for (int i = 0; i < arr.Length - 1; i++)
        {
            Debug.Assert(arr[i].CompareTo(arr[i + 1]) <= 0, "The array is not sorted!");
        }
